...
export const StudentList = () => {
  ...
  const searchPlan = (searchQuery: string) => {
    const queryObj: any = {};
  ...
  const handlePagination = async (id: Number) => {
    if (input) {
      fetchstudents({ page: id, query: input });
      setCurrentPage(id);
      scrollToTop();
    } else {
      scrollToTop();
  ...
  return (
    <div className='w-full flex flex-col gap-[25px] max-w-full px-4 sm:px-6'>
      <Header title={"Management"} />

      <section className=' bg-white font-montserrat border border-[#D6D4D4] rounded-[12px] w-full h-[770px]'>
        <article className=' py-12  px-14 flex flex-col gap-10'>
          <section className='  flex justify-between items-center '>
            <label className='input text-sm border border-[#D6D4D4] input-[5px] w-[84%] input-bordered flex items-center gap-2'>
              <input
            ...
            <div className=' flex gap-4 items-center'>
              ...
              <select className='select  border-[1px] text-[#585858] font-[500] text-sm border-[#D6D4D4]  select-bordered w-full max-w-xs'>
          ...
          <section className=' grid grid-cols-[1fr_1fr_1fr_1fr_1fr_0.3fr] gap-y-6 px-1'>
            <label className=' text-[#585858] font-[500] text-base '>
              Name
            </label>
            <label className=' text-[#585858] font-[500] text-base'>
              Mid Name
            ...
            {studentsData.students?.map((student: any, i: any) => (
              <button className=' contents  hover:bg-[#1CA2BB]' key={i}>
                <div className=' flex gap-8 text-[#414141] font-[500] text-sm'>
                  <span>{i + 1}</span>

                  {student.firstName}
                </div>

                <div className=' font-[500] text-base text-[#414141]'>
                  {student.secondName}
                </div>
                <div className=' font-[500] text-base text-[#414141]'>
                  {student.lastName}
                </div>
                <div className=' font-[500] text-base text-[#595959]'>
                  {student.regNo}
                </div>
                <div className=' font-[500] text-base text-[]'>class</div>
                ...
                {pageNumbersToShow?.map((pageNumber: any) => (
                  <button
                    ...
                    className={`py-3 px-4 text-[#1F2937] ${
                      pageNumber === currentPage ? " bg-[#D1D5DB]" : ""
