...
const DashBoard = () => {
  ...
  const fetchAllStudent = async () => {
    try {
      ...
      setDashData((prev) => ({
        ...prev,
        registered: response.data.data.total,
      }));
    } catch (error) {
      toast.error("Failed to fetch data");
    }
  };
  const fundedStudents = async () => {
    try {
      const response = await getRegisteredStudents();

      setDashData((prev) => ({
        ...prev,
        fullPaid: response.data.studentNumber,
      }));
    } catch (error) {
      toast.error("Failed to fetch data");
    }
  };
  const fetchScanInfo = async (currentDate:any) => {
    try {
     
      const response = await getScanInfo(currentDate);
  ...
  const cards = [
    {
      ...
      value: `${
        dashData.granted >= "0" ? dashData.granted : "Failed to fetch"
      }`,
      svg: (
        <svg
          width='18'
    ...
    },
    {
      identifier: "DENIED",
      status: `${
        dashData.denied >= "0" ? "UPDATES SYNCED" : "NETWORK ERROR..."
      }`,
    ...
    },
    {
      ...
      value: `${
        dashData.registered ? +dashData.registered : "Failed to fetch"
      }`,
      svg: (
        <svg
          width='18'
  ...
  return (
    <div className='w-full  max-w-full px-4 sm:px-6'>
      <section className='flex flex-col sm:flex-row mb-7  tems-center justify-between p-4 border-[2px] border-[#D1CFCF] rounded-lg'>
        <h2 className='text-[#475053] font-medium text-base sm:text-lg mb-2 sm:mb-0'>
          DashBoard
        </h2>
        <p className='text-[#5D6568] font-semibold text-sm sm:text-base'>
      ...
      <div className='mb-8 flex justify-between items-center'>
        ...
        <button className=' text-white btn bg-blue-500 w-[10%]'>Search</button>
      </div>
      ...
      <div className=' items-end flex gap-8 mt-24 '>
        <DashboardGraph />
        <Calender />
      </div>
    </div>
